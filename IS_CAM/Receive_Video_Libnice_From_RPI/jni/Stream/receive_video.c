...
{
	GstElement 	*pipeline, *nicesrc, *tee,
	...
	if (!pipeline || ! nicesrc || !tee || !display_queue ||
	...
	gst_bin_add_many (GST_BIN (pipeline), nicesrc, tee, display_queue, capsfilter,
	...
	(q1_pad = gst_element_get_static_pad (display_queue, "sink")) == NULL)
	{

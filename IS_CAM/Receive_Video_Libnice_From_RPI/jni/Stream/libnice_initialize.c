...
static gboolean is_connect_to_rpi = FALSE;
											    	 ...
											    	 GMainContext *context)
{
	...
	g_signal_connect(G_OBJECT(agent),
						 ...
						 G_CALLBACK(libnice_new_remote_candidate),
						 NULL);
...
void libnice_new_remote_candidate  (NiceAgent *agent,
					guint      stream_id,
					guint      component_id,
					gchar     *foundation,
					gpointer   user_data)
{
	__android_log_print (ANDROID_LOG_DEBUG, TAG, "New remote candidate");
}
...
void  libnice_candidate_gathering_done (NiceAgent *agent, guint stream_id, gboolean *signal_type)
{
	...
	__android_log_print (ANDROID_LOG_INFO, TAG, "agent = %d, stream_id = %d", agent, stream_id);
...
void libnice_print_local_data (NiceAgent *agent, guint stream_id, guint component_id, gchar* local_info)
{
	...
	__android_log_print(ANDROID_LOG_INFO, TAG, "%s %s", local_ufrag, local_password);
	...
	for (item = cands; item; item = item->next)
	{
		...
		__android_log_print(ANDROID_LOG_INFO, TAG," %s,%u,%s,%u,%s",
		c->foundation,
		c->priority,
		ipaddr,

...
{
	GstElement *pipeline, *pjnathsrc, *capsfilter, *rtph264depay,
		 *ff_decodeh264, *video_view;
	GstBus *bus;
 	GstMessage *msg;
	...
	video_view = gst_element_factory_make ("xvimagesink", NULL);

	g_object_set (pjnathsrc, "icest", icedemo.icest, NULL);
	g_object_set (pjnathsrc, "address", &icedemo.rem.def_addr[0], NULL);
	...
	/// [05] Create the empty pipeline 
	pipeline = gst_pipeline_new ("pjnath_receive_pipeline");

	if (!pipeline || ! pjnathsrc || !capsfilter || !rtph264depay|| !ff_decodeh264|| !video_view) {
	g_printerr ("Not all elements could be created.\n");
	return -1;
	}

	/// Build the pipeline 
	gst_bin_add_many (GST_BIN (pipeline), pjnathsrc, capsfilter, rtph264depay, ff_decodeh264, video_view, NULL);
	if (gst_element_link_many (pjnathsrc, capsfilter, rtph264depay, ff_decodeh264, video_view, NULL) != TRUE) {
